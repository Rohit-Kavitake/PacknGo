...
exports.login = async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    ...
    return res.status(400).json({ message: 'invalid password or email', status: 'Fail' });
  }
...
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'There was error sending the mail', status: 'Fail' });
  }
};
